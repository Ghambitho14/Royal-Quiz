...
export function GameLobby({ user, onStartGame, onLogout }) {
	...
	return (
		<div className="max-w-3xl mx-auto p-4">
			...
			<div className="flex items-center justify-between mb-8 bg-white/10 backdrop-blur-xl border border-white/30 rounded-3xl p-5 shadow-2xl shadow-blue-900/20">
				<div className="flex items-center gap-4">
					<Avatar className="size-14 border-2 border-amber-400/50 shadow-xl ring-2 ring-white/20">
						...
						<AvatarFallback className={`${user.isGuest ? 'bg-linear-to-br from-gray-600 to-gray-700' : 'bg-linear-to-br from-blue-700 to-blue-900'} text-white text-lg`}>
			...
			{user.isGuest && (
				<Alert className="mb-6 bg-linear-to-r from-yellow-50/98 to-amber-50/98 border-2 border-yellow-300/50 rounded-2xl backdrop-blur-xl shadow-xl shadow-yellow-500/10">
			...
			<Card className="p-8 bg-white/98 backdrop-blur-2xl shadow-2xl border-0 rounded-3xl overflow-hidden">
				...
				<div className="absolute top-0 left-0 right-0 h-1 bg-linear-to-r from-blue-700 via-purple-600 to-amber-500"></div>
				...
				<div className="mb-8">
					...
					<div className="grid grid-cols-2 gap-4">
						<Button
							...
							className={`h-24 rounded-2xl text-base transition-all duration-300 ${
								gameMode === 'casual' 
									? 'bg-linear-to-br from-blue-600 via-blue-700 to-blue-800 hover:from-blue-700 hover:via-blue-800 hover:to-blue-900 shadow-xl shadow-blue-700/40 scale-105 border-2 border-blue-400/30' 
						...
						<Button
							...
							className={`h-24 rounded-2xl text-base transition-all duration-300 ${
								gameMode === 'ranked' 
									? 'bg-linear-to-br from-amber-500 via-amber-600 to-yellow-600 hover:from-amber-600 hover:via-amber-700 hover:to-yellow-700 shadow-xl shadow-amber-500/40 scale-105 border-2 border-amber-400/30' 
					...
					{gameMode === 'ranked' && !user.isGuest && (
						<div className="mt-4 text-center bg-linear-to-r from-amber-50 via-yellow-50 to-amber-50 py-3 px-4 rounded-xl border-2 border-amber-300/50 shadow-sm">
				...
				<Separator className="my-8 bg-linear-to-r from-transparent via-gray-200 to-transparent" />

				{/* Waiting room header */}
				<div className="flex items-center gap-3 mb-6">
					<div className="size-12 rounded-2xl bg-linear-to-br from-blue-700 via-blue-800 to-blue-900 flex items-center justify-center shadow-lg shadow-blue-700/30 ring-2 ring-blue-400/20">
				...
				<div className="space-y-5">
					...
					{!players.some(p => p.id === user.id) && players.length < 8 && (
						<Button
							...
							className="w-full bg-linear-to-r from-blue-700 via-blue-800 to-blue-900 hover:from-blue-800 hover:via-blue-900 hover:to-blue-950 h-14 rounded-xl shadow-lg hover:shadow-xl transition-all hover:scale-[1.02] border border-blue-600/30"
						>
					...
					<div className="space-y-3">
						{players.length === 0 ? (
							<div className="text-center py-16 text-gray-400 bg-linear-to-br from-gray-50 to-gray-100/50 rounded-2xl border-2 border-dashed border-gray-200">
							...
							players.map((player, index) => (
								<div 
									key={player.id}
									className="flex items-center justify-between p-4 bg-linear-to-r from-gray-50 via-white to-gray-50 rounded-2xl border-2 border-gray-100 hover:shadow-lg hover:border-blue-200 transition-all hover:scale-[1.02] group"
								>
									<div className="flex items-center gap-4">
					...
					{players.length >= 1 && (
						<Button 
							...
							className="w-full bg-linear-to-r from-blue-700 via-blue-800 to-blue-900 hover:from-blue-800 hover:via-blue-900 hover:to-blue-950 text-lg h-16 rounded-2xl shadow-xl shadow-blue-700/40 hover:shadow-2xl hover:shadow-blue-700/50 transition-all hover:scale-[1.02] font-semibold"
						>
